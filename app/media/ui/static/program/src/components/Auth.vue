...
export default {
    ...
    computed: {
        info() {
            ...
            //return error, info
            return {
                hidden: (this.info.message.length < 1),
                error: (this.info.status === "error"),
                info: (this.info.status === "info"),
                success: (this.info.status === "success")
            }
        }
    },
        ...
        register(data) {
            ...
            fetch('/api/accounts/', {
                ...
                .catch((error) => {
                    ...
                    this.info.message = 'The account could not be created for the following reason: ' + error
                })
        },
        login() {
            // TODO JWT token auth
